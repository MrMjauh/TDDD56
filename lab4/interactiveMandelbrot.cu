...
// Image data
	...
	__managed__ int	 gImageWidth, gImageHeight;
...
__global__
void computeFractalKernel(unsigned char *ptr){
...
void computeFractal( unsigned char *ptr)
{
    // map from x, y to pixel position
		...
		unsigned char *dptr;
		size_t size = gImageHeight*gImageWidth*4*sizeof(unsigned char);
		cudaMalloc( &dptr, size );

		dim3 dimBlock( blockSize, blockSize );
		dim3 dimGrid( gImageHeight/blockSize, gImageWidth/blockSize );
    ...
    computeFractalKernel<<< dimGrid, dimBlock >>> ( dptr );
		cudaEventRecord(event1, 0);
		cudaThreadSynchronize();

		cudaMemcpy( ptr, dptr, size, cudaMemcpyDeviceToHost );
		cudaFree( dptr );
